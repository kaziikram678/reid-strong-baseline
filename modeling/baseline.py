...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat,
                 ...
                 use_sa_l3=False, use_sa_l4=False, use_sg_l3=False, use_sg_l4=False):
        super().__init__()

        # ---- Backbone selection (each branch fully indented) ----
        if model_name == 'resnet18':
            ...
            self.base = SENet(
                block=SEResNetBottleneck, layers=[3, 4, 6, 3],
            ...
            self.base = SENet(
                block=SEResNetBottleneck, layers=[3, 4, 23, 3],
                groups=1, reduction=16, dropout_p=None, inplanes=64,
                input_3x3=False, downsample_kernel_size=1, downsample_padding=0,
                last_stride=last_stride
            )
            ...
            self.base = SENet(
                block=SEBottleneck, layers=[3, 8, 36, 3],
                groups=64, reduction=16, dropout_p=0.2, last_stride=last_stride
            )

        elif model_name == 'se_resnext50':
            self.base = SENet(
                block=SEResNeXtBottleneck, layers=[3, 4, 6, 3],
                groups=32, reduction=16, dropout_p=None, inplanes=64,
                input_3x3=False, downsample_kernel_size=1, downsample_padding=0,
                last_stride=last_stride
            )
            ...
            self.base = SENet(
                block=SEResNeXtBottleneck, layers=[3, 4, 23, 3],
                groups=32, reduction=16, dropout_p=None, inplanes=64,
                input_3x3=False, downsample_kernel_size=1, downsample_padding=0,
                last_stride=last_stride
            )
        ...
        if self.neck == 'no':
            ...
            raise ValueError(f"Unknown neck: {self.neck}")

    def forward(self, x):
        global_feat = self.gap(self.base(x))             # (B, C, 1, 1)
